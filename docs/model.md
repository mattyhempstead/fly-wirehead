...
The Play reveal button below the scene starts a 30-second camera sequence: a 1.4-second hold on a central fly followed by one continuous pullback to all 10,000 blocks. Station G5 is one of the four nearest the room's centre, and its origin block is one of the four central blocks in the even-sized campus grid. The opening saves the user's chosen angle and panned target, with the same visible vertical span adapted to the wider frame. Single station opens this saved shot too. Elevation gradually changes from about 37° above horizontal at the opening to 27° in the wide view, with minimal rotation and recentering as the span expands. Logarithmic zoom speed is constant through the middle of the sequence, including the room reveal. Integrating a smooth speed ramp over the first and last 6% of travel gives gentle acceleration and deceleration without an intermediate stop. The camera's distance and clipping range grow with the view, and the final framing fits the whole grid on both portrait and landscape screens. The sequence ends at the All blocks preset and can be replayed or restarted. Pause holds its clock; dragging, scrolling, or choosing a preset cancels it at the displayed pose. Starting it resumes a paused floor, including when reduced-motion preferences initially paused playback. It runs only after an explicit button or WebMCP `reveal` action; it never autoplays.
