...
Instances are ordered in concentric rings so the draw count expands only as the camera can see farther. The primary 100-room block and the other 99 blocks use separate batches so their appearance can be timed independently. Neighbouring rooms fade in across visible spans 28–110, replacing the later, shorter 65–95 interval. The outer grid appears immediately at full opacity at span 900, after the complete first block is visible at span 800. It has no fade-in. At the largest scale, all 10,000 rooms and 100 plinths use seven draw calls. The original room keeps its complete animated geometry until a span of 4,000 units, near the end of the pullback. This avoids the earlier visible replacement at span 180; the room occupies only about a dozen pixels in the normal preview before its proxy takes over. Detailed fly posing then stops until the room is visible again. Its screen atlas and shared neural observations continue. Copies share the source imagery; the large scene is a rendering demonstration, not 960,000 independent simulations.
